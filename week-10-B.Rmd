...
mod_1_wfl <- workflow() %>%
  add_model(linear_reg() %>%
              set_engine("lm")) %>%
  add_recipe(recipe(reg_chg ~ treatment + poverty_n, data = week_10_train))
...
Overfitting - fitting data to week 10 train too well makes it harder to apply the model to other datasets
...
mod_1_wfl %>%
  fit(data = week_10_train) %>%
  predict(new_data = tibble(treatment = "local", poverty_n = 1.5))
...
95% confidence that true change in registration for a polling station w/ poverty 1.5 SDs above the mean and in the treatment group should be between 0.034 and 0.053. 
